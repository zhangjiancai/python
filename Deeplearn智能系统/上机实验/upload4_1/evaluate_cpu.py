...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            #wrong!tensorflow: weights are [in_channels, height, width, out_channels]
            ...
            current = tf.add(tf.matmul(tf.cast(current, tf.float32), kernels.reshape(-1,4096)), bias.reshape(-1))
        elif name  == 'fc7':
            kernels, bias = weights[i][0][0][0][0]
            current = tf.add(tf.matmul(current, kernels.reshape(-1,4096)), bias.reshape(-1))
        elif name  == 'fc8':
            kernels, bias = weights[i][0][0][0][0]
            current = tf.add(tf.matmul(current, kernels.reshape(-1,1000)), bias.reshape(-1))
        net[name] = current 
...
def _conv_layer(input, weights, bias):
    ...
    #stride=1,pad=same
    return tf.nn.conv2d(input=input, filter=weights, strides=[1,1,1,1], padding='SAME') + bias
